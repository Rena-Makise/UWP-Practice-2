...
public class Library
{
    ...
    private static readonly SymmetricKeyAlgorithmProvider algorithm = SymmetricKeyAlgorithmProvider.OpenAlgorithm(SymmetricAlgorithmNames.AesCbcPkcs7);
    private static readonly CryptographicKey key = algorithm.CreateSymmetricKey(CryptographicBuffer.CreateFromByteArray(values));
    private static readonly IBuffer iv = values.AsBuffer();

    // 파일의 컨텐츠를 TextBox로 가져오는 메소드이다.
